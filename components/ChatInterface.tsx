  ...
  const renderContent = (content: string) => {
    ...
    return parts.map((part, idx) => {
      if (part.startsWith('[BACKSTAGE]')) {
        ...
        // Check if there is JSON Payload
        const hasPayload = inner.includes('PAYLOAD:');
        
        return (
          <div key={idx} className="my-3 p-3 bg-slate-100 border-l-4 border-slate-500 rounded-r-md text-sm font-mono text-slate-700 shadow-sm relative overflow-hidden">
            <div className="flex items-center gap-2 mb-2 text-xs font-bold text-slate-500 uppercase bg-slate-200/50 p-1 rounded w-fit">
              <FileText className="w-3 h-3" />
              SYSTEM KERNEL
            </div>
            
            <div className="whitespace-pre-wrap text-xs opacity-80">{inner}</div>
    ...
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-white">
        ...
        <div className="flex justify-center">
            <span className="text-xs text-gray-400 font-mono">Orchestrator v5.0 • Audited & Patched • JSON Protocol Active</span>
        </div>
        ...
        <div className="flex items-center gap-2 max-w-4xl mx-auto">
            <div className="flex-1 relative">
                <textarea
                    ...
                    placeholder="Назначьте задачу, запросите статус или проведите ребалансировку..."
