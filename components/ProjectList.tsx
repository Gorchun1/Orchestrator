...
const ProjectList: React.FC<ProjectListProps> = ({ projects, activeProjectId, onSelectProject }) => {
  return (
    <div className="h-full bg-gray-900 text-white flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
        ...
        {projects.map(p => (
            ...
            >
                <FolderOpen className={`w-4 h-4 ${activeProjectId === p.id ? 'text-indigo-200' : 'text-gray-600'}`} />
                <span className="truncate">{p.name}</span>
            </button>
        ))}
        
        <button className="w-full mt-4 flex items-center justify-center gap-2 border border-dashed border-gray-700 rounded-md p-2 text-xs text-gray-500 hover:text-white hover:border-gray-500 transition-colors">
